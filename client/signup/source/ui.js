...
function bindCountryDropdown({ root, toggle, toggleLabel, hiddenInput, initialCountry }) {
    let isOpen = false;
...
function renderTermsCheckbox() {
    ...
    input.setAttribute(
        "aria-describedby",
        "checkbox-terms-validation-messages",
    );
...
function renderSignupTypeHatch() {
    ...
    mobileHatch.setAttribute(
        "data-qa",
        "signup-type-button-mobile-form-hatch",
    );
    ...
    desktopHatch.setAttribute(
        "data-qa",
        "signup-type-button-pc-form-hatch",
    );

    const desktopText = document.createElement("span");
    desktopText.className = "text-body";
    desktopText.appendChild(
    ...
    loginLink.href =
        "https://www.example.com/ab/account-security/login";
