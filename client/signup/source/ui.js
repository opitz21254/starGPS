...
const setupForm = () => {
  ...
  addClasses(wrapper, ["span-6", "mt-3x"]);
  stampScopedAttr(wrapper);

  const labelElement = renderLabelElement("first-name", "First name");
  ...
  const firstNameInput = inputWrapper;

  // End Replace

  const lastNameInput = renderTextField({
    name: "last-name",
    label: "Last name",
  ...
  const emailElementInput = renderTextField({
    name: "email",
